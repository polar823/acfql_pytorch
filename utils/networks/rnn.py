...
Reference: /home/xukainan/much-ado-about-noising/mip/networks/rnn.py
"""
...
class RNN(BaseNetwork):
    """RNN network for flow-based policy learning.

    Supports LSTM and GRU with uniform frequency time embeddings.
    ...
    Args:
        ...
        rnn_hidden_dim: Hidden dimension of RNN (default: 256)
        rnn_num_layers: Number of RNN layers (default: 2)
        rnn_type: Type of RNN ("LSTM" or "GRU", default: "LSTM")
        timestep_emb_dim: Dimension of time embedding (default: 128)
        max_freq: Maximum frequency for time embedding (default: 100.0)
        disable_time_embedding: Whether to disable time embedding (default: False)
        dropout: Dropout rate (default: 0.1)
        mlp_layer_dims: Hidden dimensions for final MLP layers (default: None)
    """
    ...
    def forward(
        ...
        Args:
            ...
            condition: (b, To, obs_dim) - observation condition

        Returns:
            output_data: (b, Ta, act_dim) - predicted action sequence
            scalar_output: (b, 1) - predicted scalar value
        """
        ...
        if condition is not None:
            if isinstance(condition, dict):
                ...
                condition_flat = torch.cat(condition_parts, dim=-1)
            else:
...
class VanillaRNN(BaseNetwork):
    ...
    def forward(
        ...
        Args:
            x: (b, Ta, act_dim) - action sequence
            s: (b,) - source time parameter (concatenated directly)
            t: (b,) - target time parameter (concatenated directly)
            condition: (b, To, obs_dim) - observation condition
...
if __name__ == "__main__":
    ...
    y_vanilla, scalar_vanilla = model_vanilla(x, s, t, condition)
    print(f"Output shape: {y_vanilla.shape}")
    print(f"Scalar shape: {scalar_vanilla.shape}")
