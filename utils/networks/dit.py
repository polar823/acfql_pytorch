"""DiT (Diffusion Transformer) network for policy learning.

Reference: /home/xukainan/much-ado-about-noising/mip/networks/sudeepdit.py
"""
...
class _TimeNetwork(nn.Module):
    """Time embedding network with sinusoidal features."""
...
class _ShiftScaleMod(nn.Module):
    """Shift and scale modulation layer for DiT."""
...
class _ZeroScaleMod(nn.Module):
    """Zero-initialized scale modulation layer for DiT."""
...
class _DiTDecoder(nn.Module):
    """DiT decoder block with adaptive layer normalization."""
...
class DiT(BaseNetwork):
    """Diffusion Transformer (DiT) network for flow-based policy learning.
    ...
    Args:
        ...
        timestep_emb_type: Type of timestep embedding (default: "positional")
        timestep_emb_params: Additional parameters for timestep embedding
        disable_time_embedding: Whether to disable time embedding (default: False)
    """
    ...
    def __init__(
        ...
        **kwargs,  # Accept extra kwargs for compatibility
        ...
        print(
            f"DiT parameters: {sum(p.numel() for p in self.parameters()):,}"
        )
    ...
    def forward(
        ...
        Args:
            x: (b, Ta, act_dim) - action sequence
            s: (b,) - source time parameter
            t: (b,) - target time parameter
            condition: (b, To, obs_dim) or dict - observation condition
        ...
        if condition is not None:
            if isinstance(condition, dict):
                # Handle dict observations by flattening all values
                condition_parts = []
                ...
                cond_flat = torch.cat(condition_parts, dim=-1)
            else:
                cond_flat = torch.flatten(condition, 1)  # (b, To * obs_dim)

            # Adjust if condition size doesn't match expected
            expected_size = self.obs_dim * self.To
            if cond_flat.shape[-1] != expected_size:
                # Pad or truncate
                if cond_flat.shape[-1] < expected_size:
                    padding = torch.zeros(batch_size, expected_size - cond_flat.shape[-1], device=device)
                    cond_flat = torch.cat([cond_flat, padding], dim=-1)
                else:
                    cond_flat = cond_flat[:, :expected_size]
...
if __name__ == "__main__":
    ...
    y2, s2 = model_no_time(
        x, torch.rand(batch_size), torch.rand(batch_size), condition
    )
    ...
    print("\nTesting with dict condition:")
    dict_condition = {
        "state": torch.randn(batch_size, To, obs_dim),
    }
    y_dict, scalar_dict = model(x, s, t, dict_condition)
    print(f"Output with dict condition shape: {y_dict.shape}")

    print("=" * 50)
    print("DiT test completed!")
    print("=" * 50)
