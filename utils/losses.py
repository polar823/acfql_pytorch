...
class OptimizationConfig:
    """Configuration for optimization and loss computation."""
    seed: int = 0
    loss_type: str = "flow"
...
def mf_loss(
    ...
    mu = -0.4
    sigma = 1.0
    normal_time_pair = mu + sigma * torch.randn(batch_size, 2, device=device)
    time_pair = torch.sigmoid(normal_time_pair)
    s = torch.minimum(time_pair[:, 0], time_pair[:, 1])
    t = torch.maximum(time_pair[:, 0], time_pair[:, 1])

    # Apply instant mask
    instant_mask = torch.bernoulli(
        torch.full((batch_size,), 0.2, device=device)
    )
    s = torch.where(instant_mask.bool(), t, s)
    ...
    _, ds_xst = flow_map.jvp_s(s, t, Is, obs_emb)
    u_xst, _ = flow_map.net(Is, s, t, obs_emb)
    t_diff = t-s
...
def imf_loss(
    ...
    Is = interp.calc_It(s, act_0, act_1)
    vs = flow_map.get_velocity(s, Is, obs_emb)
    u_xst, _ = flow_map.net(Is, s, t, obs_emb)
    with torch.no_grad():
        # Difference 1: use vs instead of dot_Is
        # Difference 2: also disable gradient for jvp_x
        # compute jvp
        _, duds = flow_map.jvp_s(s, t, Is, obs_emb)
