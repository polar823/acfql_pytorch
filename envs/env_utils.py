...
def make_env_and_datasets(env_name, frame_stack=None, action_clip_eps=1e-5, image_size=84, use_eye_in_hand=True):
    ...
    Args:
        ...
        image_size: Image size for image-based environments.
        use_eye_in_hand: Whether to use eye-in-hand camera for image-based environments.

    Returns:
        A tuple of (env, eval_env, train_dataset, val_dataset).
        For image-based envs, also returns shape_meta as 5th element.
    """
    shape_meta = None  # Only set for image-based envs
    ...
    # Return shape_meta for image-based envs
    if shape_meta is not None:
        return env, eval_env, train_dataset, val_dataset, shape_meta
